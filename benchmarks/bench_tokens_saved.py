...
This bench (rather than a plain pytest test) serves as a worked example
referenced in the documentation. The JSON shape the bench produces is
the schema the documentation points at. Committing
``benchmarks/baselines/tokens_saved.json`` gives the docs a stable
artefact to point at and gives the ``--check-regression`` gate a real
