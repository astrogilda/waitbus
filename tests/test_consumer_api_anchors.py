"""Lint test: backticked anchors in docs/CONSUMER_API.md and supplementary docs.
...
Coverage scope: rules 1 and 2 run over ``docs/CONSUMER_API.md`` and any
``docs/launch-articles/*.md`` files present in the repository — those are
the rot-prone anchors that point into the codebase.  Rule 3 (bare-file
existence) runs over ``CONSUMER_API.md`` ONLY: supplementary articles
legitimately name external client-config files (``config.toml``,
``claude_desktop_config.json``, ``cline_mcp_settings.json``, ...) that do
not — and must not — exist in this repository, so a file-existence
assertion over those articles is wrong by construction.
"""
...
# Supplementary articles (if present) also cite codebase anchors and must obey rules 1 and 2.
