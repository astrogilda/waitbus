...
- **Python target:** 3.11 minimum (`requires-python = ">=3.11"`). CI tests 3.11, 3.12, and 3.13.
- **Ruff rule set:** `["E", "F", "I", "N", "UP", "B", "SIM", "RUF"]`. `tests/*` allows `E402` for property-test deferred imports.
- **Line length:** 120 characters.
- **Mypy strict** on `waitbus/` only; tests are deliberately excluded (`[tool.mypy] files = ["waitbus"]`).
...
`socket.socket(AF_UNIX, SOCK_SEQPACKET)` and re-emits each non-heartbeat frame
...
- `broadcast.py`, `_doorbell.py`, and `read_events.py --watch` use
  `AF_UNIX SOCK_SEQPACKET`. Darwin returns `ENOPROTOOPT`. These test modules
  carry `pytestmark = pytest.mark.skipif(sys.platform != "linux", ...)`.
- `broadcast._peer_uid` uses `SO_PEERCRED` (Linux-only), resolved via
...
- The MCP server (`waitbus/mcp.py`) subscribes to the broadcast bus; on macOS
  it emits one info-severity notification and exits cleanly because the
  `SOCK_SEQPACKET` surface is unavailable.
...
- No internal jargon (no session references, plan section numbers, or
  commit-series labels).
...
| `waitbus-broadcast.socket` | AF_UNIX SOCK_SEQPACKET activation socket |
...
The three publish steps (PyPI, MCP Registry, GitHub Release) are intentionally
