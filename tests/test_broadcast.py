...
async def test_replay_lag_eviction_closes_subscriber_with_real_fd(
    ...
    try:
        ...
        # Leak regression: the subscriber must be gone from the map.
...
async def test_heartbeat_loop_evicts_lagged_subscriber(tmp_db_path: Path) -> None:
    ...
    The test exercises ``_close_subscriber(reason="heartbeat_lag")`` directly;
    this covers the heartbeat-loop decision (``lag_count >= LAG_LIMIT`` after
    a heartbeat send -> evict) that drives eviction.
    """
...
def test_broadcast_accepts_explicit_socket_and_doorbell_paths(tmp_path: Path) -> None:
    ...
    The explicit-injection seam that lets a self-contained in-process caller (e.g.
    a demo) bind a daemon to a temp runtime dir without mutating the
