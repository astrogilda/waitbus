...
def _get_fresh(monkeypatch: pytest.MonkeyPatch) -> waitbus._config.CiStatusConfig:  # type: ignore[name-defined]  # noqa: F821
...
def test_config_defaults_match_field_defaults(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Construct CiStatusConfig with no env, no TOML; assert every field default."""
    for var in (
        "WAITBUS_PROM_OWNER",
        "WAITBUS_PROM_REPO",
        "WAITBUS_LOG_LEVEL",
