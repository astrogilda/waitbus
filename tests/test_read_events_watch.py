...
def state_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
    """Redirect the unified BookmarkCursor state dir to a tmp path.

    Post-convergence ``--watch`` resumes via ``BookmarkCursor``, whose
    cursor files live under the platformdirs/``WAITBUS_STATE_DIR``
    state root (not a per-(owner,repo) cache). The bespoke
    ``read_events.cursors_dir`` monkeypatch is gone with the deleted
    cursor model.
    """
    d = tmp_path / "state"
...
async def test_watch_persists_cursor_via_bookmark(running_daemon: _DaemonPaths, state_dir: Path) -> None:
    """DELTA D2: resume token is persisted through the unified
    BookmarkCursor (the deleted bespoke (owner,repo) file model)."""
    daemon, paths = running_daemon
...
async def test_watch_resume_from_cursor_skips_already_seen(running_daemon: _DaemonPaths, state_dir: Path) -> None:
    ...
    # Pre-populate the unified bookmark as if a prior watch saw d-a.
    bookmark = read_events.watch_bookmark_name("o", "r")
