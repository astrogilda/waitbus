...
and fans rows out to subscribed sockets within a single millisecond of
commit. The companion `_doorbell.py` connects an AF_UNIX SOCK_STREAM socket and
...
and sweeps the events table via `SELECT ... WHERE event_id > :cursor
LIMIT 500` on each wake. On Linux the daemon uses an ``os.eventfd`` as
...
def _validate_subscribe_token(token: object) -> str | None:
    ...
    comparison against the keyring-provisioned secret. Raises ValueError
...
class Broadcast:
    ...
    def __init__(
        ...
        *,
        ...
        Looks up the optional broadcast token from GNOME Keyring at
    ...
    def _check_subscribe_token(self, raw_token: object, peer_uid: int) -> bool:
        """Constant-time compare against the keyring-provisioned token.
...
def main() -> int:
    ...
    Configuration is environment-driven via ``CiStatusConfig``. The only CLI sugar the
