...
def await_predicate(
    ...
    * Every decoded non-heartbeat frame is handed to ``decide`` and, if
    ...
    try:
        ...
        # unit (see DECISION_LOG / pr-monitor-no-unit-option-b).
        # Installing a signal handler in a library function would also
        # leak process-global state into the open-core seam; the MCP
...
# it is a pure typed-API module the open-core boundary keeps
