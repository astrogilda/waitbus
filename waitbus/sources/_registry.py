...
#: Indexed by source name. Used by :func:`waitbus source list` to
...
def _validate_common(spec: SourceSpec) -> None:
    ...
    (name / event-type regex, local-primitive forbidden-field prefixes) already ran in
...
def register_source(spec: SourceSpec) -> SourceSpec:
    ...
    code that already holds a :class:`SourceSpec` -- a built-in synthesizer,
    a test, a benchmark, or any in-process producer -- registers it
    ...
    local-primitive forbidden-field prefixes) already ran in ``SourceSpec.__post_init__``
    ...
    if not isinstance(spec, SourceSpec):
        raise PluginContractError(
            ...
            "Construct a SourceSpec (which validates its fields, including the local-primitive "
...
def plugin_publishers() -> Mapping[str, VerifiedPublisher | None]:
    ...
    Used by ``waitbus source list`` to surface "where did this plugin
...
def _clear_for_test_isolation() -> None:
    ...
    The leading underscore signals "non-public API, cross-module access by
    tests/benchmarks is the documented exception" per the project's
    underscore-with-test-access convention.
    """
    global _DISCOVERED
    with _REGISTRY_LOCK:
