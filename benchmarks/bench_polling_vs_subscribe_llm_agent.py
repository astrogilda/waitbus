...
history stays intact and any historical reclassification (a prior-session
finding) is the operator's reconcile step, not this bench's.
...
def _build_limitations() -> list[str]:
    ...
    block verbatim plus additional findings recorded in prior sessions
    (Bonferroni boundary, stress-test-historical-not-modified, Anthropic
    prefix cache, pilot gate). Recorded so a reader of the verdict sees
    every acknowledged-but-not-closed scope item.
    """
    return [
