...
The v0.3.0-era separate ``filters.json`` file was retired in v0.4.0; three config surfaces collapsed to two (env + TOML).
"""
...
def _load_filters() -> dict[str, Any]:
    ...
    from ``config.toml`` via ``CiStatusConfig.get_config()``. The default
    ...
    Replaces the v0.3.0-era ``filters.json`` file (retired in v0.4.0).
    """
...
# =============================================================
# CiStatusServer subclass, subscription registry, and per-session
...
# --- CiStatusServer subclass ----------------------------------------------


class CiStatusServer(Server[Any, Any]):
...
def _tool_definitions() -> list[types.Tool]:
    ...
    Every entry sets ``title`` (per Round 3 verified VS Code/Cursor
    prefer rendering title over name), an ``inputSchema``, and an
...
def _register_handlers(server: CiStatusServer) -> None:
...
def build_server() -> Server[Any, Any]:
    ...
    Returns a ``CiStatusServer`` instance (a Server subclass that
    ...
    server = CiStatusServer(name="waitbus", version=PACKAGE_VERSION)
