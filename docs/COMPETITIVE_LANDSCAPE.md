...
*Framed 2026-05-28 against a primary-source market verification (MCP spec
roadmap + competitor repositories read directly). It states plainly what each
neighbour does better and where waitbus does not overlap. Star counts and spec
dates move; treat them as directional and re-check before quoting.*
...
a wait predicate added. Its defensible corner is narrow and specific:
...
- **External / OS events the agent did not initiate (a pushed-branch CI run, an
  ...
  That puts this corner on a clock — but it narrows it rather than closing it. An
  MCP trigger is a single server pushing to its own clients; waitbus is a
  workstation-local bus that normalizes heterogeneous OS/external events and fans
  them out across *every* harness on the box. The durable posture is to consume an
  MCP trigger as just another source when it ships, not to race it.
...
These projects compete with waitbus for *mindshare* in the
"local bus for agents" category far more than for *function*. The category is
real and growing; the direct local-bus field is still early.
...
an orchestrator, not a competitor to it. Cross-machine relay is deliberately
out of scope for the waitbus core (see `docs/STRATEGY.md` and the open-core
...
shipped, foundational IPC. The precedent is credibility, not a threat model:
waitbus is "D-Bus for agents," and what it adds on top is a durable replay log
