...
async def test_pull_tools_answer_over_stdio(tmp_path: Path) -> None:
    ...
    async with stdio_client(params) as (read, write), ClientSession(read, write) as session:
        ...
        ci = await asyncio.wait_for(
            session.call_tool("get_ci_status", {"repo": "astrogilda/waitbus"}),
        ...
        # resources/read dispatch -- the article documents resources too, and
        # they were equally dead over the wire before the fix.
