...
daemon fans each row to every connected consumer within a millisecond. Your
agent blocks on `waitbus wait` (including across sources at once with
`--all-of`/`--first-of`) with zero polling and idle CPU until the thing it
cares about happens, and `waitbus on <predicate> -- <command>` runs a command
the moment it does. Because every agent on the box shares that one bus, it
...
reasons over what the bus stored. It is the local opt-in
...
- **You want zero-token, zero-poll waiting** as a daily habit across many local
...
```bash
uvx waitbus demo          # one command, no install (pip install waitbus also works)
```

> Install today is `uvx` / `uv tool install` (or `pip`). A single self-contained binary and a Homebrew formula are on the near-term roadmap — not shipped yet.
...
```
  REMOTE ingress (webhook)            IN-PROCESS ingress (emit())
  GitHub / Alertmanager (HTTPS)       pytest · docker · fs · plugins · any agent
         |                                   |
         v                                   |  emit() writes the row directly
  waitbus listener serve  :9000/webhook        |  (no listener, no network hop)
  (HMAC-SHA256 verify)                        |
         |  INSERT OR IGNORE                  |  INSERT OR IGNORE
         +------------------+      +----------+
                            v      v
                       SQLite events DB        ~/.local/state/waitbus/*.db (Linux)
                            |
                            | doorbell (AF_UNIX SOCK_STREAM ping, 1 byte; Linux coalesces via eventfd)
                            v
                  waitbus broadcast serve   AF_UNIX SOCK_STREAM hub
                            |                Linux: /run/user/$UID/waitbus/broadcast.sock
                            |                macOS: <tempdir>/waitbus/broadcast.sock
                  fan-out to N subscribers (length-prefix framed, 4-byte big-endian)
                            |
              +-------------+------------+-------------------------+
              v             v            v                         v
        read_events     pr_monitor   Python MCP server      any `waitbus wait`
         (CLI tail)     (PR state)   (waitbus mcp serve)       subscriber, incl. agents
```
...
broadcast doorbell so subscribers wake within a millisecond regardless of which
ingress produced it.
...
```python
from waitbus._db import insert_event
from waitbus._paths import BROADCAST_SOCKET, DB_PATH
```
...
```bash
# Live tail — one line per event; Monitor-friendly cadence
...
- **Sub-second `workflow_job` failure detection.** Catches matrix-cell failures
  the moment GitHub delivers the event — no waiting for the parent
...
waitbus is released under the MIT License. See [LICENSE](LICENSE) for the
full text, and [docs/STRATEGY.md](docs/STRATEGY.md) for the rule that the
core stays MIT.
...
- [docs/STRATEGY.md](docs/STRATEGY.md) — long-term vision, open-core outlook, and why waitbus does not seek venture funding
...
For readers interested in *why* waitbus is shaped the way it is — small, local,
MIT-licensed, framework-neutral, and deliberately not chasing the
hosted-relay or venture path — see [docs/STRATEGY.md](docs/STRATEGY.md). It
records the long-term vision (a universal local-wait primitive where CI is
merely source #1), the realistic ceiling (the loved-OSS class alongside
`ripgrep`, `fzf`, and `direnv`), the open-core option that adoption may one
day unlock, and the binding rule that the core stays MIT forever.
