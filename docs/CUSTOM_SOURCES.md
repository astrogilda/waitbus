...
internal to the MIT core; the open-core separability rule is enforced
by `tests/test_sourcespec_open_core.py`.
...
waitbus does not vet third-party plugin code. The MIT core publishes
...
the MIT waitbus core itself is installed via the same `pip install
...
# or, on macOS:
launchctl kickstart -k gui/$UID/xyz.waitbus.broadcast
launchctl kickstart -k gui/$UID/xyz.waitbus.listener
```
...
convention with verb-specific differences. See [`docs/EXIT_CODES.md`](
EXIT_CODES.md) for the full per-verb reference and operator-scripting
examples.

## Related documents

- [`../PROJECT_CONTEXT.md`](../PROJECT_CONTEXT.md) -- stable project
  conventions and repository layout.
