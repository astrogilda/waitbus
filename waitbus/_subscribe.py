...
Open-core boundary: this surface is part of the MIT local primitive. It
MUST stay purely local --
no relay / account / multi-tenant / network-coordination parameters,
behaviour, or imports. A future out-of-repo network tier layers its own
transport *on top of* this (yielding the same :class:`EventFrame`); it never
reaches into this module. The ``socket_path`` argument is the sole seam: a
local proxy daemon is reached by passing a different path, with zero change
to the logic here. ``tests/test_subscribe_open_core.py`` enforces the
no-network-symbol contract by AST walk (mirroring
``tests/test_sourcespec_open_core.py``).
...
def wait_for(
    ...
    *,
    ...
    Args:
        ...
        socket_path: Override the broadcast socket path (the network-tier /
            test seam).
