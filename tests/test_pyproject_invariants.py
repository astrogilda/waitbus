...
2. The ``WaitbusConfig.settings_customise_sources`` callback parameter
...
These four invariants share a failure mode: a commit that passes the
test runner while structurally breaking the configuration surface (a
dropped section header, a parameter rename, a missing trigger, a drifted
exclusion list). The structural lints here catch that class at write-time
rather than after a broken regression has already ridden a release.
"""
...
def test_settings_customise_sources_signature_matches_pydantic_settings_contract() -> None:
    ...
    """
    from waitbus._config import WaitbusConfig

    sig = inspect.signature(WaitbusConfig.settings_customise_sources)
    ...
    assert parameter_names == expected, (
        f"WaitbusConfig.settings_customise_sources parameter names drifted from the "
