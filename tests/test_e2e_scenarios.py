...
# ---------------------------------------------------------------------------
# Scenario 3: MCP agent workflow over multi-source
# ---------------------------------------------------------------------------


@pytest.mark.skip(
    reason=(
        "Full MCP subprocess exercise lives in tests/test_mcp_e2e.py "
        "(synthetic-frame variant). Extending it to all four sources is "
        "tracked separately so this test stays focused on the broadcast "
        "+ predicate seams the other scenarios exercise."
    )
)
async def test_e2e_mcp_agent_workflow_multi_source() -> None:
    """Placeholder: multi-source variant of test_mcp_e2e.

    The MCP subprocess + frame-injection scaffolding is already wired
    in ``tests/test_mcp_e2e.py`` for a single workflow_run frame;
    extending it to four-source coverage is a follow-up. The skip
    is intentional rather than a stub: pytest's skip reason carries
    the rationale so a future contributor can land the variant
    without re-deriving the MCP test plumbing.
    """
