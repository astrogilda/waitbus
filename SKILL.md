...
any legacy event data from `~/.claude/events/` if present to the new
...
- `waitbus read-events watch` prints one stdout line per matching
  event — the canonical shape for arming under the `Monitor` skill
  primitive. Filters default to the current git checkout's
...
- `waitbus mcp serve` subscribes to the same broadcast socket and re-emits
  ...
  without an explicit `Monitor` invocation. The broadcast daemon runs
...
the `Monitor` skill primitive), e.g. `waitbus pr-monitor tick --pr 7`.
