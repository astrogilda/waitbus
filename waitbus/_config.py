...
Merges operator-visible settings from two sources (highest priority first):
  ...
  3. Field-level defaults encoded in :class:`WaitbusConfig`.
...
class _TomlSettingsSource(PydanticBaseSettingsSource):
    ...
    object (which is discarded after WaitbusConfig is built).
    """
    ...
    def __init__(self, settings_cls: type[BaseSettings], toml_path: Path) -> None:
        ...
        # through WaitbusConfig.settings_customise_sources → __init__.
...
class WaitbusConfig(BaseSettings):
    ...
    # === MCP server filter (replaces the retired filters.json) ================
    ...
    @classmethod
    def from_environment_and_toml(cls) -> WaitbusConfig:
...
def get_config() -> WaitbusConfig:
    ...
    """
    return WaitbusConfig.from_environment_and_toml()
