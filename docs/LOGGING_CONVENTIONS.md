...
(per the `DEC-...-LOG-CONSOLIDATION-NO-STRUCTLOG` decision in
`DECISION_LOG.md`). This page is the field-naming + discipline contract
...
exit codes. HTTP → `status`. Application/process exit → `code`. The
2026-05 audit found `etag_poll.py` drifted (`code=exc.code` for an
`HTTPError.code` alongside `status=200` elsewhere); that drift was
fixed to `status=` so every HTTP-status field in the codebase uses one
key.
...
This page is intentionally short. It is enforced by review, not by a
lint today; if field drift recurs, a positive test asserting the
reserved-key set is the next step.

---

## Related Documents

- [`../PROJECT_CONTEXT.md`](../PROJECT_CONTEXT.md) -- project conventions
