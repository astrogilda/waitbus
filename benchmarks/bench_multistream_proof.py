...
Per-event granularity (verified by claude-cli empirical probe).
...
# Bootstrap parameters. 10k BCa resamples is the standard recommendation for tight CIs;
# cost is ~50-100ms per CI at N=50/side -- negligible
...
def _partition_windows(
    ...
    *,
    ...
    signal whenever loaded utime exceeded the jiffie floor -- empirically,
    real loaded utime samples of 40-60 ms exceed the 10 ms jiffie
    threshold and cause over-rejection when the filter is symmetric.
    The asymmetric filter (idle-only) preserves the
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Daemon CPU-side perturbation detection under the multi-producer / multi-subscriber swarm.",
