...
def coalesce_replay(
    ...
    **The caller owns the socket lifecycle** (mirrors
    ...
    # ships the same separation in 2024-2026 production
    # (client-library projection over an unchanged broker wire), so
    # the design holds long-term even after a future remote-subscriber
    # surface arrives.
    for frame in merged:
        emit(frame)
