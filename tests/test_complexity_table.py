...
def test_scripts_dir_has_no_d_or_worse_functions(scripts_d_or_worse: str) -> None:
    ...
    same policy anticipates the trigger condition for this
    second-tier check::

        If the ratchet needs extending (e.g. a bench grows to
        operator-visible C(20)+ and needs review), extend
        `tests/test_complexity_table.py`'s radon invocation rather
        than carving exceptions in this table.

    This guards a real regression class: a ``scripts/`` main can jump
    from D(22) to E(37) in a single change. This test catches
...
def test_benchmarks_dir_has_no_d_or_worse_functions(benchmarks_d_or_worse: str) -> None:
    ...
    (the C+ table) and ``scripts/`` (the D+ empty-set). Without it the
    measurement suite sits outside both quality gates: a bench function
    like ``bench_multistream_proof.py::_build_verdict`` can accrete to
    F(118) -- six times the operator-visible C(20) line -- unnoticed.
    The remedy is to extend this test's radon invocation rather than
    carving exceptions in the table, so this is the empty-set form (no
    per-function exemption dict), matching the scripts/ tier.
