"""Construction-time smoke test for ``CiStatusConfig``.
...
on the very first ``CiStatusConfig()`` construction.  This test catches
...
def test_ci_status_config_constructs_against_cleared_env(
    ...
    """Constructing ``CiStatusConfig()`` against a cleared environment
    ...
    cfg = _config.CiStatusConfig.from_environment_and_toml()
