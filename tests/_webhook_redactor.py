...
the GH-9951654-7992-a1 incident background that makes header-class
...
_REPO_PLACEHOLDER = "astrogilda/waitbus-demo-fixture"

# Placeholder used for all redacted HTTP headers per GH-9951654-7992-a1 policy.
_HEADER_REDACTED = "<redacted-per-GH-9951654-7992-a1-policy>"

# Headers that must be scrubbed.  The full X-GitHub-* family is covered
# because the GH-9951654-7992-a1 incident leaked webhook secrets in the
# X-Hub-Signature-256 / X-Hub-Signature headers across every delivery for
# ~4 months; the broader X-GitHub-* family carries delivery and installation
# IDs that are correlation vectors in their own right.
...
# CCN: nested-data-traversal complexity class — per-field
# rule application across deeply nested octokit/webhooks payload shapes.  The
# function's branch density mirrors the data shape; refactoring into a
# `_REDACTION_RULES` declarative table is the documented next step if/when a
# fifth top-level field-family lands.
...
def redact_headers(headers: dict[str, str]) -> dict[str, str]:
    ...
    Covers the header allowlist from GH-9951654-7992-a1: X-Hub-Signature-256
