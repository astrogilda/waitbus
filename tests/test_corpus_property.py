...
# malformed/edge inputs that triggered prior audit findings: missing
...
def test_emit_corpus_event_never_raises_counter_iff_none(
    ...
    - At most one row lands per call (DB layer's INSERT OR IGNORE may
      drop a duplicate-delivery_id event; that silent-dedup-drop class
      is a separate v0.5.1 backlog item).
    ...
    for i, event in enumerate(events):
        ...
        # drop a row when two events share a delivery_id; that's a
        # separate "silent dedup-drop" class (v0.5.1 backlog: surface it
        # via a counter analogous to corpus_decode_fallthroughs).
        # The corpus-contract invariant is about JSON-parse failures only.
...
def _count_rows(db_path: Path) -> int:
    ...
    The same pattern is the likely root cause of the MCP test-isolation flake in
