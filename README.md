...
```mermaid
%%{init: {'theme':'base','themeVariables':{'fontSize':'20px'},'flowchart':{'nodeSpacing':25,'rankSpacing':40,'padding':16}}}%%
flowchart TB
    SRC["Sources<br/>GitHub · Alertmanager (webhook)<br/>pytest · docker · fs · agents · plugins"]
    LS["listener serve<br/>HMAC verify · etag-poll fallback"]
    DB[("SQLite event store<br/>seq PK · INSERT OR IGNORE")]
    BD["broadcast daemon<br/>AF_UNIX fan-out · doorbell wake<br/>length-prefixed frames"]
    CLI["CLI<br/>wait · on · top · read-events"]
    MCP["MCP server<br/>mcp serve"]
    SDK["Python SDK<br/>subscribe · wait_for"]
    ...
    classDef core fill:#dce8f3,stroke:#2f6690,stroke-width:2px,color:#000000
    classDef store fill:#f3ecdc,stroke:#8a6d2f,stroke-width:2px,color:#000000
    class SRC,LS,CLI,MCP,SDK node
    class BD core
    class DB store
```
