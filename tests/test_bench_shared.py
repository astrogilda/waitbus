...
# ---------------------------------------------------------------------
# scan_balanced_json — N4 / Hole 3 closure.
# ---------------------------------------------------------------------
...
def test_scan_balanced_json_oversized_input_raises() -> None:
    """A 2 MiB blob aborts at the length cap (N4)."""
    blob = "x" * (2 * 1024 * 1024)
...
# ---------------------------------------------------------------------
# read_daemon_cpu_ns — N3 comm-with-parens closure.
# ---------------------------------------------------------------------
...
def test_read_daemon_cpu_ns_parses_comm_with_parens(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Reproduces N3: a daemon named via ``prctl(PR_SET_NAME)`` with parens
    in the name would break a naive ``line.split()``. The fixture line
    follows ``man 5 proc`` exactly with utime=100, stime=50 in jiffies.
    """
