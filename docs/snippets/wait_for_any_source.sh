...
waitbus wait --source pytest --match outcome=fail --timeout "$TIMEOUT" >/dev/null 2>&1 &
pids+=($!)

waitbus wait --source docker --match exit_code=1 --timeout "$TIMEOUT" >/dev/null 2>&1 &
pids+=($!)

waitbus wait --source github --match conclusion=failure --timeout "$TIMEOUT" >/dev/null 2>&1 &
pids+=($!)
