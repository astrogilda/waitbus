...
``config.toml`` file consumed by :class:`waitbus._config.CiStatusConfig`.
...
from . import _paths
from ._config import CiStatusConfig, _flatten_toml
...
def _validate_config_file(toml_path: Path) -> list[dict[str, Any]]:
    """Validate ``toml_path`` against ``CiStatusConfig``.
    ...
    try:
        ...
        CiStatusConfig.model_validate(flat)
...
def _emit_json_schema() -> str:
    """Return the canonical JSON schema for CiStatusConfig as a pretty string.
    ...
    """
    schema = CiStatusConfig.model_json_schema()
...
def _toml_value_literal(value: Any) -> str:
    ...
    Supports the subset of types the current CiStatusConfig uses (str,
    ...
    coercion on purpose. Every CiStatusConfig field defaults to one of the
...
def run_schema(fmt: str, out_path: Path | None) -> int:
    ...
    if fmt == "json":
        ...
        text = _emit_toml_template(CiStatusConfig.model_json_schema())
    else:
        print(
