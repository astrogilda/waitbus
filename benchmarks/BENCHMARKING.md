...
2. **Polling counterfactual latency.** The wall-clock between a
   ...
   This is the comparison the documentation cites, not "we are
...
half-width of ~+/-0.3 percentile points -- tight enough to withstand external scrutiny.
...
A daily-driver workstation is acceptable in principle
...
The Hetzner API token should be set in the `HCLOUD_TOKEN` environment variable.
...
The github row's excluded network leg is the externality that the per-
source comparison matrix in the documentation annotates explicitly; for
