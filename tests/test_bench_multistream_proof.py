...
def _has_openai_key() -> bool:
    """True iff secret-tool returns a non-empty key for the openai service."""
...
def test_smoke_mode_no_real_llm_produces_verdict_with_expected_shape(tmp_path: Path) -> None:
    ...
    for row in verdict.windows:
        ...
        # The full-run path is exercised by the production Hetzner
        # invocation rather than smoke.
        if row.arm == "loaded":
