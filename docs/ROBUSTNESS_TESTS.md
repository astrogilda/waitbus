...
suspend-recovery thresholds. A clean soak run is a precondition for a
public release flip; it is not a substitute for the correctness layer.
See `docs/SOAK_TEST.md` for the operator manual.
...
| `test_subscribe_envelope_hygiene` latent-RED: the hygiene scan matched prose quotes of the frame pattern inside narrative documents, producing false positives for over 24 hours undetected | May 2026 | `d69a38b` | `d69a38b` from May 2026 | `uv run pytest tests/test_subscribe_envelope_hygiene.py -v` |
...
- [`SOAK_TEST.md`](SOAK_TEST.md) -- operator manual for the 24-hour
  longevity soak on Hetzner; a pass is a precondition for the public
  release flip.
