"""Polling-cycle latency for ``os.stat()`` file-mtime polling.

Measures the wall-clock interval a polling agent would experience when
it detects a file change by calling ``os.stat(path).st_mtime_ns`` on a
1-second interval.

This bench is the *counterfactual* to waitbus's filesystem TTFAE: instead
of a push notification from watchdog/inotify, the agent polls the file's
mtime at a fixed interval.

Measurement definition
----------------------
- ``t_inject_ns``: recorded by the workload thread immediately before
  it calls ``os.utime(path, None)`` to advance the file's mtime.
...
The polling interval is the floor on latency.  With a 1-second poll
interval the expected p50 is ~500 ms and p99 is ~990 ms; the
distribution is approximately uniform over [0, poll_interval].

Per-iteration protocol
----------------------
1. Reset: snapshot ``prev_mtime_ns = os.stat(path).st_mtime_ns``.
2. Workload thread waits a random delay in ``[0, poll_interval)``
   then calls ``os.utime(path, None)`` and records ``t_inject_ns``.
3. Polling loop calls ``os.stat(path).st_mtime_ns`` every 1.0 s;
   when it observes a change it records ``t_observe_ns``.
...
::
    ...
    # Smoke (~25 seconds)
    uv run python -m benchmarks.bench_polling_baseline_fs --smoke

    # Production baseline (~8 minutes, two phases)
    taskset -c 2,3 uv run python -m benchmarks.bench_polling_baseline_fs \\
        --output benchmarks/baselines/polling_baseline_fs.json

    # Regression check in CI
    uv run python -m benchmarks.bench_polling_baseline_fs --check-regression
"""
...
_BENCH_NAME = "polling_baseline_fs"
_DEFAULT_N = 500
_DEFAULT_WARMUP = 50
_SMOKE_N = 50
_SMOKE_WARMUP = 5
...
class _WorkloadThread(threading.Thread):
    ...
    then calls ``os.utime(path, None)`` to advance the mtime and records
    ``t_inject_ns`` into ``inject_queue``.

    After each inject the thread blocks on ``_next_event`` until the
    main thread calls :meth:`request_next`.  This coordination prevents
    the workload thread from racing ahead of the polling loop and losing
    samples.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name="waitbus-bench-workload-fs", daemon=True)
    ...
    def run(self) -> None:
        for _ in range(self._n_total):
            ...
            if self._stop_event.wait(delay):
                return
            # Advance the mtime to the current wall clock.
            self._path.touch()
    ...
    *,
    ...
    try:
        for i in range(n_total):
            prev_mtime_ns = watched_path.stat().st_mtime_ns
            worker.request_next()

            while True:
                time.sleep(_POLL_INTERVAL_S)
                current_mtime_ns = watched_path.stat().st_mtime_ns
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Measure polling-cycle latency for os.stat() file-mtime polling.",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=("path to write the result JSON (default: benchmarks/results/polling_baseline_fs_<host>_<ts>.json)"),
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
    ...
    with tempfile.TemporaryDirectory(prefix="waitbus-bench-polling-fs-") as tmp_str:
    ...
    result = collect_result(
        ...
        extra={
            "smoke": args.smoke,
            "poll_interval_s": _POLL_INTERVAL_S,
            "method": "os.stat(path).st_mtime_ns",
