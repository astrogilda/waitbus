...
* :class:`SourceSpec` is a frozen :class:`msgspec.Struct` plugin authors
  ...
  open-core separability invariant (enforced in-tree by
  ``tests/test_sourcespec_open_core.py``) to third-party subclasses.
...
Open-core boundary: :class:`SourceSpec` MUST NEVER carry a field whose
name signals a network-coordination / relay / multi-tenant role. The
forbidden-name list is enforced (a) by
``tests/test_sourcespec_open_core.py`` as a binding AST-walk test
of the open-core separability rule against ``SourceSpec`` itself, AND
...
# ``tests/test_sourcespec_open_core.py`` against SourceSpec itself AND
...
# open-core separability rule (the ``SourceSpec`` surface must not assume
...
class SourceSpec(msgspec.Struct, frozen=True, kw_only=True):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        """Extend the open-core separability invariant to subclasses.

        ``tests/test_sourcespec_open_core.py`` enforces the forbidden-
            ...
            for forbidden_prefix in _FORBIDDEN_SOURCESPEC_FIELD_PREFIXES:
                if field_name.startswith(forbidden_prefix):
                    raise TypeError(
                        ...
                        f"open-core separability rule forbids relay / auth / "
