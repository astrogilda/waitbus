...
def _select_pydantic_model(*, real_mode: bool = False) -> tuple[Any, str]:
    ...
    if _openai_key_present():
        try:
            ...
            return OpenAIChatModel(REAL_OPENAI_MODEL_ID), PROVIDER_OPENAI_GPT_4_1_NANO
...
def auth_smoke_check(frameworks: Iterable[str] | None = None) -> dict[str, str]:
    ...
    spawning any drivers -- the fail-fast policy aborts rather than skipping a missing CLI.
...
def parse_claude_envelope(stdout: str) -> TokenUsage | None:
    ...
    Source-of-truth shape (verified against live driver output):
