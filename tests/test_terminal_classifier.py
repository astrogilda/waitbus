...
frozensets and for the per-source ``entity_key`` / ``is_terminal``
seam consumed by the coalesced replay mode (and, later, by an MCP
Tasks adapter). The tests pin:

* the frozenset values are byte-identical to the canonical constants
  used by `pr_monitor.AGG_SQL`;
* `wait._exit_code_if_terminal` agrees with the canonical frozensets;
...
def test_frozensets_byte_identical_to_known_values() -> None:
    """The canonical frozenset values must match these known constants."""
