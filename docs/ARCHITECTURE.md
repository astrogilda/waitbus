...
1. Seed `cursor = MAX(event_id)` on startup.
2. On every doorbell ping (or batch of pings, coalesced via FIFO
   drain), `SELECT * FROM events WHERE event_id > :cursor ORDER BY
   event_id LIMIT 500`.
...
MAX(event_id), so a restart never reissues already-broadcast
frames. Subscribers that want the historical backlog send
...
`CiStatusConfig` at daemon startup and held on the broadcast
...
- `init` — bootstrap state dirs, run schema migration via
  ...
  `etag_state.json`, transparently migrate the legacy
  `~/.claude/events/` tree onto the platformdirs target if one
  exists.
...
- `config validate [PATH]` — loads `PATH` (or the platformdirs
  ...
  through `tomllib` and the `CiStatusConfig` pydantic-settings model,
...
Configuration knobs on `CiStatusConfig`:
