...
The ``watch_delta``-marked tests pin behaviours that are expected to change
when ``--watch`` migrates to the shared ``open_subscriber`` / ``BookmarkCursor``
engine. Rewrite those tests if that migration lands; every other test in this
file must stay green verbatim.
"""
...
async def test_watch_clean_eof_returns_0(running_daemon: _DaemonPaths) -> None:
    """Daemon closing the connection -> int return 0 (monitoring re-arm)."""
    daemon, paths = running_daemon
    rc_holder: list[int] = []
...
async def test_watch_cursor_persistence_delta(
    ...
    """Post-migration behaviour: ``--watch`` resume is now the unified
...
def test_watch_bespoke_cursor_api_deleted_delta() -> None:
    """Post-migration behaviour: the bespoke
